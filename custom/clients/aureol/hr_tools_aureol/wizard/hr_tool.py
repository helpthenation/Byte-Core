...
class WizardGetRecord(models.TransientModel):
    ...
    def create_employees(self):
        ...
        with NamedTemporaryFile(mode='r+b') as tempInFile:
            ...
            del data
            rows = csv.DictReader(tempInFile.file, delimiter=',', quotechar='"')
            if 'NAME' not in rows.fieldnames:
                raise ValidationError('Error Required field NAME not available')
            for row in rows:
                emp_name = row['NAME']
                wage = row['BASIC']
                trans = row['TRANS']
                ...
                car = row['CAR']
                ins = row['INS']
                employee = employee_obj.create({'name': emp_name,
                                     'birthday': '1990-01-01',
                                     'initial_employment_date': '2018-01-01'
                                     })
                # Lets Create Contract
                c_data = {'name': employee.name,
                         'employee_id': employee.id,
                         'aureol_staff_category': 'general',
                         'type_id': 1,
                         'wage': float(wage),
                         'struct_id': 1,
                ...
                contract = contract_obj.create(c_data)
                # lets create allowance
                allowance_lines = []
                if len(trans) > 1:
                    allowance_lines.append((0, 0, {
                ...
                if len(otm) > 1:
                    ammendment_obj.create({'state':'validate',
                                           'employee_id':employee.id,
                                           'category_id':1,
                                           'hr_period_id':11,
                                           'name':'Pay Slip Amendment: '+str(employee.name),
                                           'amount':otm})
