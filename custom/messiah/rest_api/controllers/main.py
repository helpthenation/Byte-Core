...
def get_fields_values_from_model(modelname, domain, fields_list, offset=0, limit=None, order=None, hash=None):
  ...
  #  cr._cnx.set_isolation_level(ISOLATION_LEVEL_READ_COMMITTED)
    ...
    if hash:
        hash = eval(hash)
        hash = {eval(k):v for k,v in hash.items()}
        domain = [('id', 'in', hash.keys())]
    ...
    if hash: # filter and get updated record
        not_updated = []
        for rec in records:
            hash_val = hash.get(rec.id)
            if hash_val != rec.last_hash:
...
access_token_expires_in = config.get('oauth2_access_token_expires_in', 600)
refresh_token_expires_in = config.get('oauth2_refresh_token_expires_in', 216000)
