...
class TestHrFiscalyear(common.TransactionCase):
    ...
    def test_get_period_from_date(self):
        ...
        period = self.period_model.get_period_from_date(
            self.company_id.id, 'monthly', '2015-01-02')
        self.assertEqual(periods[0], period)

        period = self.period_model.get_period_from_date(
            self.company_id.id, 'monthly', '2015-03-02')
        self.assertEqual(periods[2], period)
    ...
    def test_get_next(self):
        ...
        fy0 = self.create_fiscal_year(vals=vals0)
        fy0.create_periods()
        periods0 = self.get_periods(fy0)
        ...
        fy = self.create_fiscal_year(vals=vals)
        fy.create_periods()
        periods = self.get_periods(fy)
        ...
        fy2 = self.create_fiscal_year(vals=vals)
        fy2.create_periods()
        periods2 = self.get_periods(fy2)

        self.assertEqual(periods[1], periods[0].get_next())
        self.assertEqual(periods[2], periods[1].get_next())
        self.assertEqual(periods[3], periods[2].get_next())
        self.assertEqual(periods[3], periods[2].get_next(step=1))
        self.assertEqual(periods[4], periods[2].get_next(step=2))
        self.assertEqual(periods[5], periods[2].get_next(step=3))
        self.assertEqual(periods[6], periods[2].get_next(step=4))

        # ensure that we can correctly cross into the next year
        self.assertEqual(periods2[0], periods[11].get_next())
        self.assertEqual(periods2[0], periods[0].get_next(step=12))
        self.assertEqual(periods2[1], periods[1].get_next(step=12))
        self.assertEqual(periods2[2], periods[1].get_next(step=13))
