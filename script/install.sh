...
OE_VERSION="school_primary_secondary"
...
sudo adduser --system --quiet --shell=/bin/bash --home=$OE_HOME --gecos 'BYTE' --group $OE_USER
...
sudo git clone --depth 1 --branch $OE_VERSION https://github.com/butagreeza/Byte-Core.git $OE_HOME_EXT/
...
sudo cp $OE_HOME_EXT/.conf/conf.conf /etc/${OE_CONFIG}.conf
